...
## 快速开始（M004 新管线，免 API Key）

需要两个终端：

```bash
# Terminal 1 — 本地 Claude OAuth 代理（:8787）
npm run proxy

# Terminal 2 — 静态服务器（:8081，ES modules 需要 HTTP）
npm run serve
```

浏览器打开：`http://localhost:8081/vibe-origin.html?proxy=1`

1. 点 🎤 哼一段旋律（5-10 秒），再点一次停止
2. 立即看到发光种子，继而长出茎秆、叶片、彩虹花
3. 大约 26 秒后花心亮起 — 音乐开始播放
4. **你听到的 vocal 层是你自己哼的那段旋律**（经过降噪）
5. 用下方的 mood/energy slider 或 reverb/delay 旋钮实时调整

`?proxy=1` 告诉页面走本地 OAuth 代理，免 Anthropic API Key。
...
```bash
npx playwright test            # 跑全部（需要 :8081 + :8787 都在跑）
npx playwright test tests/m004-acceptance.spec.ts  # 跑 M004 验收
```
